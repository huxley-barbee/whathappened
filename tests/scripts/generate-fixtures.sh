...
sudo ausearch --format raw -k exec-log       > tests/data/syscall_execve_cwd.log || true
sudo ausearch --format raw -k open-log       > tests/data/open_path.log || true
sudo ausearch --format raw -m USER_CMD       > tests/data/user_cmd.log || true
sudo ausearch --format raw -m ADD_USER,DEL_USER > tests/data/user_add_del.log || true
sudo ausearch --format raw -m CONFIG_CHANGE  > tests/data/audit_config_change.log || true
sudo ausearch --format raw -m CRYPTO_KEY_USER > tests/data/crypto_key.log || true
